...
#define JSON_MATCHED(String, Token, Buffer) \
            ( strlen(String) == (Token)->end - (Token)->start \
...
//
// INPUT: json string, tokens, self index and indentation count
                                                                ...
                                                                unsigned int uiLevel)
{
    ...
    if ( (uiSelf = psToks[uiSelf].first_child) != -1 ) {
        uiLevel += 2;
        depth_first_dump(pcJson, psToks, uiSelf, uiLevel);
        while ( (uiSelf = psToks[uiSelf].next_sibling) != -1 ) {
            depth_first_dump(pcJson, psToks, uiSelf, uiLevel);
        }
    }
}
...
//
// INPUT:   array of methods to be called via RPC (this should be a static object)
// EFFECTS: saves a pointer to the table, and number of entries
// OUTPUT:  status code
//
// -------------------------------------------------------------------------- //
workstatus_t
rpc_install_methods(const methodtable_entry_t* psMethods, unsigned int uiNumMethods) {
    ...
    for (unsigned int i = 0; i < uiNumMethods; i++) {
        ...
        if (uiSigLen < 3 || psMethods[i].sig[0] != '(' || psMethods[i].sig[uiSigLen-2] != ')') {
            return WORKSTATUS_RPC_ERROR_INSTALLMETHODS;
        }
...
// OUTPUT:  status code
//
// TODO: iCommandLen should be a unsigned int, but for that uart_console needs to be fixed first
// -------------------------------------------------------------------------- //
static workstatus_t
rpc_parse_command(const char* const pcCommand, int iCommandLen) {
...
// -------------------------------------------------------------------------- //
static workstatus_t
rpc_validate_method(const char* const pcCommand, int* piMethod) {
    if (g_psMethodTable && g_psTokMethod && (g_psTokMethod->size == 1) ) {
        for (unsigned int i = 0; i < g_uiNumMethods; i++) {
            if ( JSON_MATCHED(g_psMethodTable[i].name, &g_psTokens[g_psTokMethod->first_child], pcCommand) ) {
                if (piMethod) { *piMethod = i; }
                return WORKSTATUS_NO_ERROR;
            }
        }
    }

    if (piMethod) { *piMethod = -1; }
...
static workstatus_t
rpc_validate_params(const char* const pcCommand, int iMethod) {
...
//
// INPUT:   command string, response buffer, response buffer size, error status code
// EFFECTS: fills the response buffer with JSON error object, up to the buffer size
// OUTPUT:  total number of character that *would have been* printed into the response buffer
//          (this may be more than it can handle, and this condition is checked in the caller)
//
// -------------------------------------------------------------------------- //
static int
...
// -------------------------------------------------------------------------- //
//
// This is the workhorse function for handling RPC requests. It takes in a string
...
//
// This method goes through a number of validation steps (parsing, rpc specifics,
...
// error or success. The returned error code does not indicate whether the response
...
// type checked.
//
// INPUT:   command string, command string len, response buffer, response buf len
...
L_done:
    ...
    if( !pcResponse || iRespMaxLen <= 0 ||
        !g_psTokId || JSON_IS_NULL(pcCommand, &g_psTokens[g_psTokId->first_child]) )
    {
        //no response
        if (pcResponse && iRespMaxLen > 0) {
            pcResponse[0] = 0;
        ...
        switch (eStatus) {
            ...
            case WORKSTATUS_PARSE_ERROR_PART:
                iTotalLen = rpc_print_error_json(pcCommand, pcResponse,
                                           iRespMaxLen, JSONRPC_20_PARSE_ERROR);
                break;
            //request malformed
            ...
            case WORKSTATUS_RPC_ERROR_INVALIDPARAMS:
                iTotalLen = rpc_print_error_json(pcCommand, pcResponse,
                                           iRespMaxLen, JSONRPC_20_INVALID_REQUEST);
                break;
            //params mismatch
            case WORKSTATUS_RPC_ERROR_PARAMSMISMATCH:
                iTotalLen = rpc_print_error_json(pcCommand, pcResponse,
                                           iRespMaxLen, JSONRPC_20_INVALIDPARAMS);
                break;
            //method not found
            case WORKSTATUS_RPC_ERROR_METHODNOTFOUND:
                iTotalLen = rpc_print_error_json(pcCommand, pcResponse,
                                           iRespMaxLen, JSONRPC_20_METHODNOTFOUND);
                break;
            //internal
	...
	//however we're checking an error message for errors... recursion follows???
    }
...
workstatus_to_string(workstatus_t eStatus) {
    switch(eStatus) {
        ...
        case WORKSTATUS_RPC_ERROR_INVALIDOUTER:
            return "WORKSTATUS_PARSE_ERROR_NOTOBJECT: outer json layer is not an object";
        case WORKSTATUS_RPC_ERROR_INVALIDVERSION:
            return "WORKSTATUS_RPC_ERROR_INVALIDVERSION: Version string must be present and equal to 2.0";
        case WORKSTATUS_RPC_ERROR_INVALIDID:
            return "WORKSTATUS_RPC_ERROR_INVALIDID: Id, if present, must be string/number/null";
        case WORKSTATUS_RPC_ERROR_INVALIDMETHOD:
            return "WORKSTATUS_RPC_ERROR_INVALIDMETHOD: Method, must be present and must be a string";
        case WORKSTATUS_RPC_ERROR_INVALIDPARAMS:
            return "WORKSTATUS_RPC_ERROR_INVALIDPARAMS: Params, if present, must be array/object";
        case WORKSTATUS_RPC_ERROR_METHODNOTFOUND:
            return "WORKSTATUS_RPC_ERROR_METHODNOTDEFINED: no such method defined, or attempting to define NULL method";
        case WORKSTATUS_RPC_ERROR_PARAMSMISMATCH:
            return "WORKSTATUS_RPC_ERROR_PARAMSMISMATCH: params mismatch for requested method";
        case WORKSTATUS_RPC_ERROR_INSTALLMETHODS:
            return "WORKSTATUS_RPC_ERROR_METHODFORMAT: RPC method install failed, check name/sig/function prototype";
        case WORKSTATUS_RPC_ERROR_OUTOFRESBUF:
            return "WORKSTATUS_RPC_ERROR_PRINTRESPONSE: Ran out of buffer printing JSON response";
